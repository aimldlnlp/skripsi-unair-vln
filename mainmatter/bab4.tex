...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth, trim={20mm 9mm 20mm 13mm}, clip]{images/nav-tugas/fig_protocol_3d_partition_en_v3.pdf}
...
Nilai \textit{Mean task step} sebesar 233{,}70 menunjukkan bahwa tugas yang berhasil dieksekusi rata-rata membutuhkan ratusan langkah.
Dalam konteks navigasi berantai, skala langkah seperti ini mengindikasikan skenario \textit{long-horizon} yang bersifat multi-tahap (multiobjek/sub-goal).
Konsekuensinya, efektivitas end-to-end tidak hanya ditentukan oleh kemampuan mencapai satu target, tetapi juga oleh stabilitas eksekusi pada beberapa episode berurutan,
sehingga error kecil pada tiap tahap berpotensi terakumulasi dan menurunkan keberhasilan end-to-end.
Benchmark modern untuk agen \textit{embodied} juga menekankan perlunya diagnosis yang lebih halus daripada \textit{final success rate} karena kegagalan dapat muncul dari
berbagai sumber (misal \textit{planning error} dan \textit{affordance error}) \parencite{li2024eai}.

Rasio rata-rata langkah mengindikasikan sekitar 3{,}21 episode per tugas (secara agregat).
Dengan beberapa episode berurutan, keberhasilan end-to-end menjadi lebih rapuh terhadap kesalahan kecil pada tiap episode: kegagalan pada satu episode saja dapat menggagalkan
keseluruhan rangkaian.
Literatur perencanaan \textit{long-horizon} juga melaporkan bahwa error perencanaan dapat bersifat akumulatif ketika horizon bertambah, sehingga performa end-to-end dapat turun
meskipun komponen lokal tampak sangat kuat \parencite{deps2023neurips,egoplan2023neurips}.

Selisih kecil antara \textit{Mean navigation success rate} (0{,}9982) dan \textit{Mean task success rate} (0{,}9945) konsisten dengan perspektif reliabilitas:
tugas end-to-end dapat dipandang sebagai rangkaian fase/episode yang seluruhnya harus berhasil. Dalam kerangka ini, ketika jumlah fase meningkat,
peluang sukses total menurun secara kumulatif walaupun peluang sukses per fase sangat tinggi. Model penilaian \textit{mission success probability} pada
\textit{phased-mission systems} dan analisis kegagalan pada sistem seri--paralel juga membahas mekanisme akumulasi risiko kegagalan pada rangkaian fase/komponen
\parencite{zhang2024msp_pms,torrado2024series_parallel_dependent}.
Meskipun analogi reliabilitas sering diasosiasikan dengan asumsi independensi antar fase, pada konteks navigasi berantai interpretasi ini digunakan sebagai justifikasi konseptual
untuk menjelaskan kecenderungan penurunan keberhasilan end-to-end ketika jumlah episode meningkat, tanpa mengklaim independensi penuh antar episode.
Intinya, semakin banyak episode yang harus berhasil secara berurutan, semakin besar peluang setidaknya satu episode gagal, sehingga keberhasilan end-to-end menurun secara kumulatif.

Gambar~\ref{fig:disc_risk_contour_dense} dapat dibaca sebagai berikut: sumbu horizontal merepresentasikan keberhasilan per episode, dan sumbu vertikal merepresentasikan panjang horizon efektif (jumlah episode).
Kontur menunjukkan nilai keberhasilan end-to-end yang diprediksi pada kombinasi kedua besaran tersebut.
Titik observasi menandai nilai yang diukur pada eksperimen, dan kedekatan titik dengan kontur nilai terukur menunjukkan konsistensi hubungan akumulasi risiko pada data.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\linewidth]{images/nav-tugas/fig_discussion_risk_contour_2d_v4_dense_left.pdf}
  \caption{Peta Kontur Akumulasi Risiko End-to-End Berbasis Keberhasilan Per Episode dan Panjang Horizon Efektif, dengan Titik Observasi dan Kontur Nilai Terukur.}
  \label{fig:disc_risk_contour_dense}
\end{figure}

Gambar~\ref{fig:disc_risk_curve_panels} memberikan pandangan komplementer: kurva sensitivitas memperlihatkan bagaimana perubahan jumlah episode efektif memengaruhi keberhasilan end-to-end pada keberhasilan episode yang tinggi,
sedangkan panel \textit{zoom} menonjolkan selisih kecil antara prediksi ilustratif dan nilai terukur di sekitar titik observasi.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\linewidth]{images/nav-tugas/fig_discussion_risk_curve_2d_v4_panels.pdf}
  \caption{Kurva Akumulasi Risiko dan Panel Diagnostik untuk Memvalidasi Konsistensi Antara Prediksi Ilustratif dan Nilai Terukur di Sekitar Titik Observasi.}
